...
TEST_CASE("UniformExplicit 2D Cylindrical Heat Solver Validation",
  ...
  SECTION("Dirichlet BC") {
    ...
    auto solution = [&](double z, double r, double t) {
      ...
      return exp(-alpha * t) * sin(lambda_z * z) *
             std::cyl_bessel_j(0, lambda_r * r);
    };
  ...
  SECTION("Nuemann BC") {
    // Neumann is --sad-- happy because his boundary condition test case is
    // ---empty--- full --:(-- :)
    double dt = 0.001;
    int Nt = 100;
    ...
    auto solution = [&](double z, double r, double t) {
      ...
      return exp(-alpha * t) * cos(lambda_z * z) *
             std::cyl_bessel_j(0, lambda_r * r);
    };
...
TEST_CASE("NonUniformExplicit 2D Cylindrical Heat Solver Validation",
  ...
  HeatSolver.T.setCoordinateSystem(Geometric(0., dL, z_s),
                                   Geometric(0., dR, r_s));
  ...
  SECTION("NonUniform Dirichlet BC") {
    ...
    for (int i = 0; i < Points.size(); i++) {
      ...
      // std::cout << "Testing " << Name[i] << "\n";
      CHECK(HeatSolver.T(temp.first, temp.second) ==
            Approx(Aplot(temp.first, temp.second)).epsilon(0.01));
    }
  }
...
TEST_CASE("Auto Adaptive Time Step", "[heatsolver][validation]") {
  ...
  NU_HeatSolver.T.setCoordinateSystem(Geometric(0., dL, z_s),
                                      Geometric(0., dR, r_s));
  U_HeatSolver.T.setCoordinateSystem(Uniform(0., L), Uniform(0., R));
  NU_Analytic.setCoordinateSystem(Geometric(0., dL, z_s),
                                  Geometric(0., dR, r_s));
    ...
    for (int i = 0; i < U_Analytic.size(0); i++) {
      for (int j = 0; j < U_Analytic.size(1); j++) {
        if (U_HeatSolver.T[i][j] + U_Analytic[i][j] == 0) {
          if (U_HeatSolver.T[i][j] + U_Analytic[i][j] ==
              U_HeatSolver.T[i][j] - U_Analytic[i][j]) {
            continue;
          } else {
            U_err += U_HeatSolver.T[i][j];
          }
        } else {
          U_err += (U_HeatSolver.T[i][j] - U_Analytic[i][j]) /
                   (U_HeatSolver.T[i][j] + U_Analytic[i][j]);
        }
      }
    }
    ...
    for (int i = 0; i < NU_Analytic.size(0); i++) {
      for (int j = 0; j < NU_Analytic.size(1); j++) {
        if (NU_HeatSolver.T[i][j] + NU_Analytic[i][j] == 0) {
          if (NU_HeatSolver.T[i][j] + NU_Analytic[i][j] ==
              NU_HeatSolver.T[i][j] - NU_Analytic[i][j]) {
            continue;
          } else {
            NU_err += NU_HeatSolver.T[i][j];
          }
        } else {
          NU_err += (NU_HeatSolver.T[i][j] - NU_Analytic[i][j]) /
                    (NU_HeatSolver.T[i][j] + NU_Analytic[i][j]);
        }
      }
    }
