...
TEST_CASE("Heat Solver Signals", "[heatsolver]") {
  SECTION("Crank-Nicholson") {
    ...
    CHECK(HeatSolver.minBC.type ==
          HeatSolvers::BoundaryConditions::Type::Temperature);
    CHECK(HeatSolver.maxBC.type == HeatSolvers::BoundaryConditions::Type::None);
    ...
    CHECK(HeatSolver.minBC.type ==
          HeatSolvers::BoundaryConditions::Type::Temperature);
    CHECK(HeatSolver.maxBC.type ==
          HeatSolvers::BoundaryConditions::Type::HeatFlux);
    CHECK(HeatSolver.minBC.f == 10);
  ...
  SECTION("Crank-Nicholson Solver") {
    ...
    SECTION("Without Source") {
      SECTION("Sink Boundary Conditions") {
        ...
        auto sol = [&](int i) {
          return exp(-alpha * Tmax) * sin(lambda * (xmin + dx * i));
        };
      ...
      SECTION("Insulating Boundary Conditions") {
        ...
        auto sol = [&](int i) {
          return exp(-alpha * Tmax) * cos(lambda * (xmin + dx * i));
        };
    ...
    SECTION("With Source") {
      SECTION("Sink Boundary Conditions") {
        ...
        auto sol = [&](int i, int n) {
          return (beta / k / lambda / lambda) * (1 - exp(-alpha * dt * n)) *
                 sin(lambda * (xmin + dx * i));
        };
    ...
    SECTION("Boundary Conditions") {
      SECTION("Constant Temperature Boundaries") {
        ...
        // set the initial temperature distribution to something close to the
        // solution so we don't have to wait so long.
        HeatSolver.T.set_f([&](auto i, auto cs) { return sol(i[0]) + 0.1; });
      ...
      SECTION("Convective Heat Flux at Max Boundary") {
        ...
        auto sol = [&](int i) {
          return (he * Tinf / (k + he * L)) * (xmin + i * dx);
        };
      ...
      SECTION("Convective Heat Flux at Min Boundary") {
        ...
        auto sol = [&](int i) {
          return (-he * Tinf / (k + he * L)) * (xmin + i * dx - L);
        };
