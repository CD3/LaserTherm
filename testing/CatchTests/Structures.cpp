...
TEST_CASE("Material Structure")
{
  ...
  f.set_f([&obj](auto x) -> std::optional<double> {
    if (obj.structure.isInside(x[0])) return obj.material.getThermalConductivity().value();
    return std::nullopt;
  });
  ...
  f.set_f([&obj](auto x) -> std::optional<double> {
    if (obj.structure.isInside(x[0])) return obj.material.getDensity().value()*obj.material.getSpecificHeatCapacity().value();
    return std::nullopt;
  });
...
TEST_CASE("Collection of Material Structures")
{
  ...
  structures.push_back( MaterialStructureType(mat, Structures::_1D::Infinite<double>()) );
  ...
  for( auto& s : structures )
  {
    rho.set_f([&s](auto x) -> std::optional<double> {
      if (s.structure.isInside(x[0]) && s.material.getDensity()) return s.material.getDensity().value();
      return std::nullopt;
    });
  }
  ...
  for( auto& s : structures )
  {
    k.set_f([&s](auto x) -> std::optional<double> {
      if (s.structure.isInside(x[0]) && s.material.getThermalConductivity()) return s.material.getThermalConductivity().value();
      return std::nullopt;
    });
  }
