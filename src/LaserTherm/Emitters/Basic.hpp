...
/**
 * An "emitter" is a heat source combined with a waveform. It knows when it is
 * "on", and it knows how to compute a source term.
 */
...
template<class HeatSourceType, class WaveformType>
void Basic<HeatSourceType, WaveformType>::addSourceTerm(source_field_type& A,
                                                        const real_type&   t)
{
  if (!this->isOn(t)) return;
