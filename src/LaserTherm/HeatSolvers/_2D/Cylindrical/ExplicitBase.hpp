...
public:
  ...
  /**
   * a function to evolve this->T in time a total of delta_t, in increments of
   * dt
   *
  ...
  /**
   * A method (roughly) implementing the Automatic Converge Time Step algorithm
   * as detailed in /doc/writups/...
   *
   * @param Delta_t is the 'initial guess' for the minimum convergent time step
   * @param t_min is the minimum time step allowed by the algorithm, 0 by
   * default
  ...
  REAL findTimeStep(REAL Delta_t, REAL t_min = 0.0, REAL alpha = 0.01) {
    ...
    // T0 = T(0), T_Dt = T(\Delta t), T_Dt2 = T(\Delta t / 2), T2_Dt =
    // T^2(\Delta t)
