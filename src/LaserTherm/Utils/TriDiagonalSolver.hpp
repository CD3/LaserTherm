 ...
 *
 * All three diagonals are assumed to be of length N. The first element of aSub and the last
 * element of aSup are not used.
 *
 *  @param	aSub   The subdiagonal (aSub(1) is the first sub diagonal element)
 *  @param	aDiag  The diagonal.
 *  @param 	aSup   The superdiagonal (aSup(N-2) is the last sub diagonal element)
...
{  
  ...
  // FORWARD SWEEP
  for(int i = 1; i < N; ++i)    // can't parallelize, each iteration depends on the last
  {
  ...
  for(int i = (N-2); i >= 0; --i)    // can't parallelize, each iteration depends on the last
  {
