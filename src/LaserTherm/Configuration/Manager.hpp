...
/**
 * Retrieves a property from the configuration tree. If the property is not
 * found, a std::runtime_error with a message showing the name of the property
 * that was requested is thrown.
 *
...
/**
 * Retrieves a property from the configuration tree and converts it to the unit
 * specified by unit. If the property is not found, a std::runtime_error with a
 * message showing the name of the property that was requrested is thrown.
 *
 * @param key property name
...
T Manager::get_quantity(const path_t& path, std::string unit) const
{
  ...
  try {
    ...
    error << "Parameter Name: " << path.dump() << "( under the node "
          << root.dump() << " )\n";
